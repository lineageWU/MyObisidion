...
开发：蚕子 QQ：312815311 更新时间：2022-3-27
***************************************************************************** */
...
var en = {
    ...
    qq: 'Link <a href="http://wpa.qq.com/msgrd?v=3&uin=312815311&site=qq&menu=yes">CanZi (QQ:312815311)</a>',
    close: 'Click here to close the window !',
    FunctionUpdate: 'Version 0.4.7 adds a bilingual interface between Chinese and English, abolition of the "Smart line" function, the conversion of English and Chinese punctuation features optimized for the "Repair error Punctuation" ...',
...
var zhCN = {
    ...
    thisPluginName: 'ZH增强编辑 ',
    helloWorld: '<b>欢迎使用 ZH增强编辑 插件！</b>',
    qq: '联系 <a href="http://wpa.qq.com/msgrd?v=3&uin=312815311&site=qq&menu=yes">蚕子（QQ：312815311）</a>',
    close: '点击此处 可关闭提示窗口......',
    FunctionUpdate: 'v0.4.7 临时测试版本。此版本\n- 增加了中英文双语界面\n- 取消了【智能换行】功能 \n- 调换MD语法格式刷快捷键 \n- 将文标点转换改为【修复错误标点】\n- 支持划选多段文本进行排序 \n- 支持中英文缩录Callout引用语法\n- ...',
    setInterTab: '📣 插入制表符「Tab」 在文本行中插入制表符效果',
    whenEnabledTab: '启用此项后，在文本行中按下 Tab 键会插入4个空格，不再整行缩进。',
...
class MyPlugin extends obsidian.Plugin {
    ...
    onload() {
        ...
        this.addCommand({
            id: 'text-background1',
            name: '转换橙色背景',
            callback: () => this.转换背景颜色(this.settings.hColor1),
            hotkeys: [{ modifiers: ["Mod","Alt"], key: "1" } ]
        });
        this.addCommand({
            id: 'text-background2',
            name: '转换红色背景',
            callback: () => this.转换背景颜色(this.settings.hColor2),
            hotkeys: [{ modifiers: ["Mod","Alt"], key: "2" } ]
        });
        this.addCommand({
            id: 'text-background3',
            name: '转换紫色背景',
            callback: () => this.转换背景颜色(this.settings.hColor3),
            hotkeys: [{ modifiers: ["Mod","Alt"], key: "3" } ]
        });
        this.addCommand({
            id: 'text-background4',
            name: '转换蓝色背景',
            callback: () => this.转换背景颜色(this.settings.hColor4),
            hotkeys: [{ modifiers: ["Mod","Alt"], key: "4" } ]
        });
        this.addCommand({
            id: 'text-background5',
            name: '转换绿色背景',
            callback: () => this.转换背景颜色(this.settings.hColor5),
        ...
        document.addEventListener('keydown',(e) =>{
            ...
            */
            ...
            if(聚焦编辑){
                ...
                if(e.key == "Backspace"){
                    console.log(当前行文本+"\n"+历史行文本);
                    if(当前行文本==历史行文本 && this.settings.twoEnter){
                        历史光标 = 当前光标;
                        编辑模式.exec("goLeft");
                        当前光标 = 编辑模式.getCursor();
                        笔记全文.replaceRange("", 当前光标,历史光标);
                    }
                }else{
                    历史行文本 = 当前行文本;
                }
                */
                ...
                if (e.key == "Enter"){
                    ...
                    if(this.settings.twoEnter &&  !代码块内 && 在列表行<0){
    ...
    转换列宽() {
        ...
        if (link1.test(所选文本)){
            ...
            所选文本 = 所选文本.replace(/(\<div[^\n]+\/div\>)/,"");
        }else{
            所选文本 = 所选文本.replace(/^([^\|]+)$/mg,"<div style='width:100px'>$1</div>");
        }
        this.替换所选文本 (所选文本);
    };
    ...
    智能符号 () {
        ...
        if(选至行尾.match(/^(\]\]|\=\=|\*\*|\~\~)/)){
            ...
            let coreAry = ["HTML","XML","VBScript|vbs","MATLAB","Java","C","C#","CSS","Objective-C","PHP","Visual Basic|vb","Basic","Python|pt","Perl","js","JavaScript|js","Delphi","Object Pascal","Ruby","Lisp","Pascal","Transact-SQL","PL","SQL","Ada","Logo","R","Lua"];
            for(var _l=0; _l<coreAry.length; _l++){
                let _coreID = coreAry[_l];
                let qcStr = _coreID.replace(/\|.+$/,"");
                if(eval("/^"+_coreID+"$/mi").test(选至行首)){
...
class editSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        */
        ...
        new obsidian.Setting(containerEl)
            .setName('📣 设置设置彩色文字及背景效果（Html语法）功能。')

        new obsidian.Setting(containerEl)
        .setName('转换文字颜色「Ctrl+Shift+1」 或 转换背景颜色「Ctrl+Alt+1」')
        .setDesc('设置文字或背景颜色值（#000000）')
        .addText(text => {
            text
        ...
        new obsidian.Setting(containerEl)
        .setName('转换文字颜色「Ctrl+Shift+2」 或 转换背景颜色「Ctrl+Alt+2」')
        .setDesc('设置文字或背景颜色值（#000000）')
        .addText(text => {
            text
        ...
        new obsidian.Setting(containerEl)
        .setName('转换文字颜色「Ctrl+Shift+3」 或 转换背景颜色「Ctrl+Alt+3」')
        .setDesc('设置文字或背景颜色值（#000000）')
        .addText(text => {
            text
        ...
        new obsidian.Setting(containerEl)
        .setName('转换文字颜色「Ctrl+Shift+4」 或 转换背景颜色「Ctrl+Alt+4」')
        .setDesc('设置文字或背景颜色值（#000000）')
        .addText(text => {
            text
        ...
        new obsidian.Setting(containerEl)
        .setName('转换文字颜色「Ctrl+Shift+5」 或 转换背景颜色「Ctrl+Alt+5」')
        .setDesc('设置文字或背景颜色值（#000000）')
        .addText(text => {
            text
